...
u32 fupd_prog(u16 x,u16 y,u8 size,u32 fsize,u32 pos)
{
	...
	if(t!=prog)
	{
		LCD_ShowString(x+3*size/2,y,240,320,size,"%",0);		
		t=prog;
		if(t>100)t=100;
...
{	
	...
	if(rval==0)//字库文件都存在.
	{  
		LCD_ShowString(x,y,240,320,size,"Erasing sectors... ",0);//提示正在擦除扇区	
...
u8 update_SDCard_font(u8* src)
{
	...
	if(rval==0)//字库文件都存在.
	{  
		LCD_ShowString(30,150,240,320,16,"Updating SD_UNIGBK.BIN  ",0);
		strcpy((char*)pname,(char*)src);				//copy src内容到pname
		strcat((char*)pname,(char*)UNIGBK_PATH); 
		res=update_SDCard_fontx(pname,0);	//更新UNIGBK.BIN
		if(res){myfree(SRAMIN,pname);return 1;}
		
		LCD_ShowString(30,150,240,320,16,"Updating SD_GBK12.FON  ",0);
		strcpy((char*)pname,(char*)src);				//copy src内容到pname
		strcat((char*)pname,(char*)GBK12_PATH); 
		res=update_SDCard_fontx(pname,1);	//更新GBK12.FON
		if(res){myfree(SRAMIN,pname);return 2;}
		
		LCD_ShowString(30,150,240,320,16,"Updating SD_GBK16.FON  ",0);
		strcpy((char*)pname,(char*)src);				//copy src内容到pname
		strcat((char*)pname,(char*)GBK16_PATH); 
		res=update_SDCard_fontx(pname,2);	//更新GBK16.FON
		if(res){myfree(SRAMIN,pname);return 3;}
		
		LCD_ShowString(30,150,240,320,16,"Updating SD_GBK24.FON  ",0);
		strcpy((char*)pname,(char*)src);				//copy src内容到pname
		strcat((char*)pname,(char*)GBK24_PATH); 
		res=update_SDCard_fontx(pname,3);	//更新GBK24.FON
		if(res){myfree(SRAMIN,pname);return 4;}
		LCD_ShowString(30,150,240,320,16,"Updating SD_OVER!    ",0);
		//全部更新好了
	}
	else
	{
		LCD_ShowString(30,150,240,320,16,"Updating SD_ERROR!  ",0);
	}
	myfree(SRAMIN,pname);//释放内存 
	return rval;//无错误.
}	
...
//拷贝指定字库到SD卡
u8 update_SDCard_fontx(u8 *fxpath,u8 fx)
{
	u32 flashaddr=0;								    
	FIL * fftemp;
	u8 *tempbuf;
	...
	tempbuf=mymalloc(SRAMIN,4096);				//分配4096个字节空间
	if(tempbuf==NULL)rval=1;
 	res=f_open(fftemp,(const TCHAR*)fxpath,FA_WRITE); //写打开文件
 	if(res)rval=2;//打开文件失败  
 	if(rval==0)	 
	{
		switch(fx)
		{
			case 0:												//更新UNIGBK.BIN
				flashaddr=ftinfo.ugbkaddr;
				datanum = ftinfo.ugbksize;
				break;
			case 1:
				flashaddr=ftinfo.f12addr;						//GBK12的起始地址
				datanum = ftinfo.gbk12size;
				break;
			case 2:
				flashaddr=ftinfo.f16addr;						//GBK16的起始地址
				datanum = ftinfo.gbk16size;
				break;
			case 3:
				flashaddr = ftinfo.f24addr;						//GBK24的起始地址
				datanum = ftinfo.gkb24size;
				break;
		} 
		...
		{	 
			if(res!=FR_OK)break;								//执行错误
			W25QXX_Read(tempbuf,offx+flashaddr,4096);		//从0开始读出4096个数据  
			res=f_write(fftemp,tempbuf,4096,(UINT *)&bwrite);		//写入数据
	  		offx += bwrite;	  
			if(offx>=(datanum-4096))	//最后一组数据
			{
				W25QXX_Read(tempbuf,offx+flashaddr,datanum-offx);		//从0开始读出4096个数据  
				res=f_write(fftemp,tempbuf,datanum-offx,(UINT *)&bwrite);		//写入数据
				break;								
			}
	 	} 	
		f_close(fftemp);		
