...
u8*const FLAC_PATH="/SYSTEM/MUSIC_PLAYER/flac.bin";		//flac的存放位置
_flac_info flinfo;	//flac结构体
...
{	 
	...
	retry=0;
	while(VS_RD_Reg(SPI_MODE)!=0x0800)	// 软件复位,新模式 （0x0800表示复位完成） 
	{
...
u8 flac_init(void)
{
	u8 t=0;
	W25QXX_Init();  
	FLACADDR = ftinfo.f24addr+ftinfo.gkb24size;	//紧跟着字库
	while(t<10)//连续读取10次,都是错误,说明确实是有问题,得更新flac了
	{
		t++;
		W25QXX_Read((u8*)&flinfo,FLACADDR,sizeof(flinfo));//读出ftinfo结构体数据
		if(flinfo.flacok==0XAC)break;
		delay_ms(20);
	}
	flinfo.flacaddr = FLACADDR + sizeof(flinfo);	//指向数据存储地址
	if(flinfo.flacok!=0XAC)return 1;
	return 0;		 
}

u8 update_flac(u8* src)
{	
	u8 *pname;
	u32 *buf;
	u8 res=0;		   
	...
	strcat((char*)pname,(char*)FLAC_PATH); 
 	res=f_open(fftemp,(const TCHAR*)pname,FA_READ); 
 	if(res)rval|=1<<4; //打开文件失败   
	myfree(SRAMIN,fftemp); //释放内存
	if(rval==0)//字库文件都存在.
	{  
		LCD_ShowString(30,170,240,320,16,"Erasing sectors... ",0);//提示正在擦除扇区	
		for(i=0;i<4;i++)	//先擦除字库区域,提高写入速度
		{
			W25QXX_Read((u8*)buf,((FLACADDR/4096)+i)*4096,4096);//读出整个扇区的内容
			for(j=0;j<1024;j++)//校验数据
			{
				if(buf[j]!=0XFFFFFFFF)break;//需要擦除  	  
			}
			if(j!=1024)W25QXX_Erase_Sector((FLACADDR/4096)+i);	//需要擦除的扇区
		}
		myfree(SRAMIN,buf);
		LCD_ShowString(30,170,240,320,16,"Updating FLAC.BIN",0);		
		strcpy((char*)pname,(char*)src);				//copy src内容到pname
		strcat((char*)pname,(char*)FLAC_PATH); 
		res=updata_flacx(pname);	//更新FLAC.BIN
		if(res){myfree(SRAMIN,pname);return 1;}
		//更新好了
		flinfo.flacok=0XAC;
		W25QXX_Write((u8*)&flinfo,FLACADDR,sizeof(flinfo));	//保存字库信息
	}
...
{							    
	...
	if(tempbuf==NULL)rval=1;
 	...
 	if(rval==0)	 
	{	
		flinfo.flacsize=fftemp->fsize;	//获得FLAC大小
		while(res==FR_OK)//死循环执行
		{
	 		res=f_read(fftemp,tempbuf,4096,(UINT *)&bread);		//读取数据	 
			if(res!=FR_OK)break;								//执行错误
			W25QXX_Write(tempbuf,flinfo.flacaddr,4096);		//从0开始写入4096个数据  
	  		offx+=bread;	  
...
//patch：patch首地址
//len：patch长度
void VS_Load_PatchX(void) 
{
	u8 temp[2];
	u16 i; 
	u16 addr, n, val; 	  			   
	for(i=0;i<flinfo.flacsize;) 
	{ 
		W25QXX_Read(temp,flinfo.flacaddr+i,2);	//读2字节数据
		addr = (u16)temp[0]<<8|temp[1];	//转换成u16数据
		i += 2;
		W25QXX_Read(temp,flinfo.flacaddr+i,2);	//读2字节数据
		n = (u16)temp[0]<<8|temp[1];
		i += 2;
