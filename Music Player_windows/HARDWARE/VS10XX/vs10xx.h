...
__packed typedef struct 
{
	u8 flacok;					//flac存在标志，0XAC，字库正常；其他，不存在
	u32 flacaddr; 			//flac的地址
	u32 flacsize;			//flac的大小	 
}_flac_info; 

extern _flac_info flinfo;	//字库信息结构体
...
u8 flac_init(void);										//初始化flac
u8 update_flac(u8* src);	
u8 updata_flacx(u8 *fxpath);		//更新FLAC
void VS_Load_PatchX(void);	//加载patch
#endif
